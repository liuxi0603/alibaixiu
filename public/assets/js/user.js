...
// 添加用户的时候会使用到头像上传，需要onchange事件
// 修改用户的时候可能会修改头像，也需要onchange事件
// 所以此处可以给添加和修改的表单的共同父级元素绑定onchange事件
// 当用户选择文件的时候
...
selectAll.on('change', function() {
    ...
    // alert(status); 测试
    // 所有用户的选中状态要和全选按钮选中状态一致
    // 获取所有的用户 #userBox是父级元素
    // js写法：document.querySelector('#userBox').querySelectorAll('input');
    $('#userBox').find('input').prop('checked', status);
});

// 因为用户的选中框是通过模板加载到页面的，所以需要事件委托
// 当用户前面的复选框状态发生改变时
